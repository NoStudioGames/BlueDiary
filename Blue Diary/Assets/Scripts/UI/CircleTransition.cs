...
namespace Collections.Shaders.CircleTransition
{
    public class CircleTransition : MonoBehaviour
    {
        ...
        {
            DrawBlackScreen();
            StartCoroutine(Transition(0.8f, 0, 1));
        }

        public void CloseBlackScreen()
        {
            DrawBlackScreen();
            StartCoroutine(Transition(0.8f, 1, 0));
        }
        public void CloseBlackScreenForCamera()
        {
            DrawBlackScreenForCamera();
            StartCoroutine(Transition(0.8f, 1, 0));
        }
        ...
        private void DrawBlackScreenForCamera()
        {
            var screenWidth = Screen.width;
            var screenHeight = Screen.height;
            // Need a target
            var playerScreenPos = Camera.main.WorldToScreenPoint(Camera.main.transform.position);
